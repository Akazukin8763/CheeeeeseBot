...
class Draw(commands.Cog):
    ...
    async def _breakfast(self, ctx: commands.Context):
        # 早餐單
        _吐司 = ["草莓吐司", "巧克力吐司", "奶油吐司", "花生吐司", "奶酥厚片",
               "煎蛋吐司", "培根蛋吐司", "火腿蛋吐司", " StinkyGlitch 蛋吐司", "薯餅蛋吐司"
               "鮪魚蛋吐司", "燻雞蛋吐司", "豬肉蛋吐司", "卡拉雞蛋吐司"]

        _厚片 = ["草莓厚片", "巧克力厚片", "奶油厚片", "花生厚片", "奶酥厚片"]

        _蛋餅 = ["原味蛋餅", "蔬菜蛋餅",
               "玉米蛋餅", "培根蛋餅", "火腿蛋餅", " StinkyGlitch 蛋餅", "熱狗蛋餅", "薯餅蛋餅",
               "鮪魚蛋餅", "燻雞蛋餅", "豬肉蛋餅", "卡拉雞蛋餅"]

        _漢堡 = ["豬肉蛋堡", "牛肉蛋堡", "燻雞蛋堡", "卡拉雞腿蛋堡"]

        _鐵板麵 = ["鐵板麵", "鐵板麵加蛋"]

        _小點 = ["荷包蛋", "德式香腸", "蘿蔔糕", "蔥抓餅", "蔥抓餅加蛋",
               "熱狗", "煎餃", "鍋貼", "雞塊", "脆薯", "薯餅"]

        _飲料 = ["紅茶", "奶茶", "豆漿", "咖啡牛奶", "豆漿紅茶", "鮮奶茶", "柳橙汁", "牛奶", "巧克力牛奶"]
        ...
        if random.randint(1, 10) <= 2:  # 是否吃鐵板麵，機率為 20%
            breakfast.extend(random.choices(_鐵板麵, k=1))
        else:  # 不以鐵板麵當主食
            main = [_吐司, _厚片, _蛋餅, _漢堡]
            random.shuffle(main)
        ...
        # 一定配飲料
        breakfast.extend(random.choices(_飲料, k=1))
